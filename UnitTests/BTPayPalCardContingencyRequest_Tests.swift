...
class BTPayPalCardContingencyRequest_Tests: XCTestCase {

    let cardContingencyRequest = BTPayPalCardContingencyRequest(contingencyURL: URL(string: "www.contingency.com")!)
    ...
    func testHandleOpenURL_whenSuccessOccurred_callsOnPaymentCompleteWithResult() {
        ...
        mockPaymentFlowDriverDelegate.onPaymentCompleteVerifier = { result, error in
            guard let result = result as? BTPayPalCardContingencyResult else { XCTFail(); return }
